...
class patch_loader(data.Dataset):
    ...
    def __init__(self, split='train', stride=30 ,patch_size=99, is_transform=True,
        ...
        if 'test' not in self.split: 
            ...
            seismic = np.load(pjoin('data','train','data_train.npz'), mmap_mode="r", allow_pickle=True)
            ...
            self.seismic = self.pad_volume(seismic)
            self.labels = self.pad_volume(labels)
        elif 'test1' in self.split:
            self.seismic = np.load(pjoin('data','data_test.npz'))
            self.labels = None
